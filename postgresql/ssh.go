...
type postgresqlSshDriver struct {
	sshClient *ssh.Client
}

// postgresqlSshDriver implements database/sql/driver.Driver interface
var _ driver.Driver = &postgresqlSshDriver{}

// postgresqlSshDriver implements github.com/lib/pq.Dialer interface
var _ pq.Dialer = &postgresqlSshDriver{}

func (p *postgresqlSshDriver) Open(name string) (driver.Conn, error) {
	return pq.DialOpen(p, name)
}

func (p *postgresqlSshDriver) Dial(network string, address string) (net.Conn, error) {
	return p.sshClient.Dial(network, address)
}

func (p *postgresqlSshDriver) DialTimeout(network string, address string, timeout time.Duration) (net.Conn, error) {
	// TODO how to correctly implement dial timeout?
	return p.sshClient.Dial(network, address)
}
...
type sshConfig struct {
	...
	// TODO can this pattern be reused?
...
// prepareSSHConfig is used to turn the *postgresql.Config provided into a
// usable *SSHConfig for client initialization.
func prepareSSHConfig(config *Config) (*sshConfig, error) {
	sshAgent, err := connectToAgent(config)
	if err != nil {
		return nil, err
	}
	...
	sshConf, err := buildSSHClientConfig(sshClientConfigOpts{
		...
		// TODO remove support for a certificate
...
func connectToAgent(config *Config) (*sshAgent, error) {
	// TODO support agent argument for the provider
	//if config.Agent != true {
	//	// No agent configured
	//	return nil, nil
	//}
	...
	return &sshAgent{
		agent: agent,
		conn:  conn,
		// TODO support AgentIdentity argument
